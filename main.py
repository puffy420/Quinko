...
scoreboard = {(rect1.left, rect1.top, rect1.width, rect1.height): 0,
              (rect2.left, rect2.top, rect2.width, rect2.height): 0,
              (rect3.left, rect3.top, rect3.width, rect3.height): 0,
              (rect4.left, rect4.top, rect4.width, rect4.height): 0}

# ...

# update the score board
for ball in balls:
    for rect, score in scoreboard.items():
        if ball['y'] < rect[1] and ball['x'] > rect[0] and ball['x'] < rect[0] + rect[2]:
            scoreboard[rect] += 1
...
while running:
    ...
    for i, ball in enumerate(balls):
        ...
        if ball['y'] > screen_height - ball_radius:
            ...
            ball['speed_x'] *= -1
        for rect in scoreboard.keys():
            if ball['y'] < rect.top and ball['x'] > rect.left and ball['x'] < rect.right:
                score[i] += 1
